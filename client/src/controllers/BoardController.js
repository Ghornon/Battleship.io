class BoardController {
	...
	droppedOnBoard(event) {
		...
		const shipLength = this._model._shipsModel
			.getShipsArray()
			.find((ship) => ship.name == shipClassName).length;
		...
		if (isHorizontal) {
			if (x - selectedShipIndex < 0 || x - selectedShipIndex + shipLength - 1 > 9) return;
			...
			for (let i = x - selectedShipIndex; i < x - selectedShipIndex + shipLength; i++) {
				const square = this._model
					.getBoard()
					.find((square) => square.x == i && square.y == y);

				if (square.isTaken) return;
		...
		if (!isHorizontal) {
			if (y - selectedShipIndex < 0 || y - selectedShipIndex + shipLength - 1 > 9) return;
			...
			for (let i = y - selectedShipIndex; i < y - selectedShipIndex + shipLength; i++) {
				const square = this._model
					.getBoard()
					.find((square) => square.x == x && square.y == i);
				squares.push(square);
			}
	...
	setComputerShips() {
		...
		while (!isDone) {
			...
			if (shipIndex >= shipsArray.length) isDone = true;
		}
	}
